...
def affine_forward(x, w, b):
    ...
    X = np.append(x.reshape(N,D), np.ones((N,1)), axis=1)
    W = np.append(w, b.reshape(1,M), axis=0)
...
def softmax_loss(x, y):
    ...
    # 1. normalize, exp x
    n_x = x - x[np.arange(num_train), y].reshape(num_train,1)
    en_x = np.exp(n_x)

    # 2. Sum of en_x in columns (N)
    column_sums = np.sum(en_x, axis =1).reshape(num_train,1)

    ###########################################################

    # 1. optimization + regularization
    loss = 1 / num_train * np.sum(np.log(column_sums))  #Broadcasting
    
    ###########################################################

    # 1. calculate ( e^cell / column_sum(e^cell) ) for each cell
    coordinate = -1 * en_x / column_sums #Broadcasting

    # 2. create correct_mask (N * D) of x,
    #    indicating the position of correct class as True
    correct_mask = np.full_like(x, False, dtype=bool)
    correct_mask[np.arange(num_train), y] = True
    coordinate[correct_mask] += 1

    # 3. optimization + regularization
    dx = -1 / num_train * coordinate 
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        var = np.sum(centered_x * centered_x, axis=0) / N
        ...
        x1 = centered_x / sigma #broadcasting
        out = gamma * x1 + beta
        
        cache = x, x1, gamma, beta, mean, sigma
...
def batchnorm_backward(dout, cache):
    ...
    N, D = dout.shape
    x, x1, gamma, beta, mean, sigma = cache

    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * x1, axis=0)
    dx1 = dout * gamma

    dvar = -0.5 * np.sum(dx1 * (x-mean), axis=0) / np.power(sigma, 3)
    dmean = -1 * np.sum(dx1, axis=0) / sigma + (-2/N) * dvar * np.sum(x-mean, axis=0) 

    dx = dx1 / sigma
    dx += (1/N) * np.tile(dmean, (N, 1))
    dx += (2/N) * dvar * (x - mean)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    x1 = np.transpose(x, axes=[0, 2, 3, 1]).reshape((N*H*W, C))
    x2, cache = batchnorm_forward(x1, gamma, beta, bn_param)
    out = np.transpose(x2.reshape(N, H, W, C), axes=[0, 3, 1, 2])
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dout1 = np.transpose(dout, axes=[0, 2, 3, 1]).reshape((N*H*W, C))
    dx1, dgamma, dbeta = batchnorm_backward_alt(dout1, cache)
    dx = np.transpose(dx1.reshape(N, H, W, C), axes=[0, 3, 1, 2])
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    N, C, H, W = x.shape
    K = C // G
    group_shape = N*K, G*H*W

    x1 = x.reshape(group_shape)
    
    ############################################################################
    mean = np.sum(x1, axis=1, keepdims=True) / group_shape[1]
    centered_x1 = x1-mean
    var = np.sum(centered_x1*centered_x1, axis=1, keepdims=True) / group_shape[1]
    sigma = np.sqrt(var + eps)
    
    x2 = centered_x1 / sigma
    ############################################################################
    
    x3 = x2.reshape(x.shape)

    out = x3 * gamma + beta
    cache = x2, x3, sigma, gamma
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x2, x3, sigma, gamma = cache
    
    dgamma = np.sum(dout * x3, axis=(0,2,3), keepdims=True)
    dbeta = np.sum(dout, axis=(0,2,3), keepdims=True)
    
    group_shape = x2.shape
    temp = (dout * gamma).reshape(group_shape)
    ############################################################################
    temp1 = np.sum(temp, axis=1, keepdims=True)
    temp2 = np.sum(temp * x2, axis=1, keepdims=True)
    
    dx = (temp - (temp1 + x2*temp2) / group_shape[1]) / sigma
    ############################################################################
