...
def svm_loss(x, y):
    ...
    # 1. num_train = N, num_dimension = D, num_classes = C 
    (num_train, num_classes) = x.shape

    # 2. create correct_mask (N * D) indicating the position of correct class
    correct_mask = np.ones_like(x, dtype=bool)
    correct_mask[np.arange(num_train), y] = False

    ###########################################################
    
    # 1. divide x into two with correct_mask 
    #    x_noCorrectClass (N * D-1)
    #    x_CorrectClass   (N * 1)
    x_noCorrectClass = x[correct_mask].reshape((num_train, num_classes-1))
    x_CorrectClass = x[~correct_mask].reshape((num_train, 1))

    # 2. calculate scores (N * D-1) from above
    #    remember loss is not calculated in the position of correct class
    scores = x_noCorrectClass - x_CorrectClass + 1  # broadcasting, 1 is delta
    scores = np.where(scores < 0, 0, scores)        # max(0,___)
    
    # 3. calculate loss from scores
    ...
    calc_table = np.zeros_like(x)                         #value == 0       i == j
